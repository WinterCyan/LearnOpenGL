...
int main() {
    ...
    // shaders
    MyShader pbrShader(PROJECT_DIR"vs.pbr", PROJECT_DIR"fs.pbr", NULL);
    ...
    pbrShader.setVec3("albedo", .1f, .1f, .1f);
...
//    unsigned int roughnessTex = loadTexture(PROJECT_DIR"hdr/roughness.png",false);
    ...
    irradianceMapShader.setMat4("projection",cubemapProjection);
    for (int i = 0; i < 6; ++i) {
        cubeMapShader.setMat4("view", cubemapViews[i]);
