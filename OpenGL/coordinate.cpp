...
int coordinate(){
    ...
    glm::mat4 view = glm::translate(glm::mat4(1.0), glm::vec3(.0f, .0f, -2.f));
    glm::mat4 projection = glm::perspective(glm::radians(45.f), (float)width/height, .1f, 100.f);
    unsigned int transLocation = glGetUniformLocation(shader.ID, "trans");
    glUniformMatrix4fv(transLocation, 1, GL_FALSE, glm::value_ptr(projection*view*model));
