...
vec3 fresnelSchlick(float cosTheta, vec3 specular)
{
    ...
    float VH = clamp(cosTheta, 0.0, 1.0);
    return specular + (1.0-specular) * pow(2, (-5.55473*VH-6.98316)*VH);
}

//vec3 fresnelSchlick(float cosTheta, vec3 metallic)
//{
...
//    vec3 F = fresnelSchlickRoughness(max(dot(N, V), 0.0), metallic, roughness);
    vec3 F = fresnelSchlick(max(dot(N, V), 0.0), metallic);
